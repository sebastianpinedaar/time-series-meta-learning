...
mode = "EVAL-WFT" #HYP/EVAL
capacity = "LOW"

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
meta_info = [  ("HR", 32, 50, 13), ("BATTERY", 20, 50, 3), ("POLLUTION", 5, 50, 14)]
lambda_per_dataset = {"POLLUTION-LOW": 0.001, "HR-LOW": 0.001, "BATTERY-LOW":0.0001,
                  "POLLUTION-HIGH":0.001, "HR-HIGH": 0.0001, "BATTERY-HIGH":0.0001}
...
def train(vrada, domain_train_loader, domain_val_loader, lambda1 = 0.1):
...
for dataset_name, window_size, task_size, x_dim in meta_info:
    ...
    for trial in range(0,4):
        ...
        if mode=="HYP":
            ...
            model_file = output_directory+"50-model.pt"
            train_idx, val_idx, test_idx = split_idx_50_50(test_data.file_idx)
            n_domains_in_test = np.max(test_data.file_idx)+1
            n_domains_in_train = np.max(train_data.file_idx)+2

            test_loss_list = []

            for domain in range(n_domains-1):
                ...
                x_test = domain_test_data.x
                y_test = domain_test_data.y
                temp_train_data = domain_train_data + DomainTSDataset(x=x_test[train_idx[domain]], 
            ...
            new_model_file = output_directory+"model_wft.pt"
            model_file = output_directory+"model.pt"
            ...
            for task_id in range(0, (n_tasks-horizon-1), n_tasks//100):
                ...
                temp_x_train = test_data_ML.x[task_id][:int(task_size*0.9)]
                temp_y_train = test_data_ML.y[task_id][:int(task_size*0.9)]
                
                temp_x_val = test_data_ML.x[task_id][int(task_size*0.9):]
                temp_y_val = test_data_ML.y[task_id][int(task_size*0.9):]
